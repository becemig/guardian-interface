...
func _get_cluster_category(nodes):
    var node_str = " ".join(nodes).to_lower()
    if "stress" in node_str or "strain" in node_str: return "Stress/Strain"
    if "sleep" in node_str or "wave" in node_str or "recovery" in node_str: return "Sleep/Recovery"
    if "autonomic" in node_str or "vagus" in node_str or "regulation" in node_str: return "Autonomic"
...
func _derive_guardian_state(payload):
    ...
    if sleep >= 0.0 and sleep < 0.35: return "Fatigued"
    if hrv >= 0.70 and respiration > 0.0 and respiration <= 14.0: return "Regulated"
    if respiration >= 18.0: return "Stress Load"
    if reaction >= 0.60: return "Cognitive Load"
    if hrv >= 0.55: return "Adaptive"
    return "Monitoring"
