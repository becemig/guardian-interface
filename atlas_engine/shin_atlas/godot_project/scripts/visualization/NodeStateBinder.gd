...
func bind_state_layers(graph: Node, manager: Node) -> void:
    ...
    if visualization_manager != null:
        if visualization_manager.has_signal("visual_packet_routed"):
            if not visualization_manager.visual_packet_routed.is_connected(_on_packet_routed):
                visualization_manager.visual_packet_routed.connect(_on_packet_routed)

        if visualization_manager.has_signal("visual_frame_updated"):
    ...
    print("[NODE STATE BINDER] Dual-pipeline routing active.")


func _on_packet_routed(packet: RefCounted) -> void:
    ...
    for node_id in somatic_graph.active_node_registry.keys():
        ...
        if node != null:
            node.scale = Vector3.ONE * float(packet.confidence)
            node.set("current_hrv", float(packet.hrv))
            node.set("state_signature", str(packet.guardian_state))
