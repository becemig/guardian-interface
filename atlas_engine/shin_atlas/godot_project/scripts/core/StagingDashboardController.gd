...
var protocol_manager: Node = null
...
func _ready():
    ...
    protocol_manager = preload("res://scripts/core/ProtocolManager.gd").new()
    protocol_manager.name = "ProtocolManager"
    add_child(protocol_manager)
    protocol_manager.protocol_phase_changed.connect(_on_protocol_phase_changed)
    protocol_manager.advance_phase()
...
func _on_protocol_phase_changed(protocol_snapshot):
    print("[Controller Protocol Snapshot]")
    print(protocol_snapshot)
