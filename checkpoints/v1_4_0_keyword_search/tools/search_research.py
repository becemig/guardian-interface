...
from guardian_interface.archive.search import search_nodes
...
def search_keyword(data, query):
    results = search_nodes(data, query)
    print(f"Keyword '{query}' -> {len(results)} document(s):")
    print()
    for node in results:
        print_node(node)
...
def main():
    ...
    parser.add_argument("--search", type=str, help="Keyword search across title, summary, tags")
    args = parser.parse_args()

    data = load_index()

    if args.search:
        search_keyword(data, args.search)
    elif args.uid:
