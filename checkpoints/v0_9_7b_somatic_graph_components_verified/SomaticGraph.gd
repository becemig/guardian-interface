...
@export var node_placeholder_scene: PackedScene
...
func generate_anatomy_graph(meridian_json_path: String = "") -> void:
    ...
    if node_placeholder_scene == null:
        node_placeholder_scene = load("res://scenes/visualization/NodeVisual.tscn")
...
func _build_scene_topology(nodes_array: Array) -> void:
    ...
    for node_info in nodes_array:
        ...
        var point_node = node_placeholder_scene.instantiate()
        point_node.initialize_node_identity(node_id, str(node_info.get("name", "")))
    ...
    print("[SOMATIC GRAPH] Component scene tree generated. Count: ", active_node_registry.size())


func get_node_by_index(index: int) -> Node:
