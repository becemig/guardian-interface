...
var telemetry_model: TelemetryProvider = null
var summary_engine: SummaryEngine = null


func bind_telemetry_model(model: TelemetryProvider) -> void:
	telemetry_model = model
	print("[SessionManager] Valid TelemetryProvider interface bound.")


func bind_summary_engine(engine: SummaryEngine) -> void:
	summary_engine = engine
	print("[SessionManager] SummaryEngine bound successfully.")
...
func request_start_session(participant_id: String = "anonymous") -> bool:
	...
	emit_signal("session_started", {
		"participant_id": telemetry_model.get_participant_id(),
		"session_id": telemetry_model.get_session_id()
	})
...
func request_end_session() -> void:
	...
	if telemetry_model != null:
		var raw_contract_payload: Dictionary = telemetry_model.get_raw_session_summary_data()

		if summary_engine != null:
			summary_engine.build_summary_contract(raw_contract_payload)
		else:
			print("[SessionManager] SummaryEngine not bound. Raw contract payload: ", raw_contract_payload)
