...
signal dashboard_data_updated(summary)
...
var session_start_time_unix: int = 0
...
func initialize_new_session(new_participant_id: String = "anonymous"):
    ...
    session_start_time_unix = int(Time.get_unix_time_from_system())
    hrv_accumulator = 0.0
    respiration_accumulator = 0.0
...
func log_current_snapshot_to_disk():
    ...
    hrv_accumulator += float(snapshot.get("hrv", 0.0))
    respiration_accumulator += float(snapshot.get("respiration_rate", 0.0))
    file.close()
...
func get_raw_session_summary_data() -> Dictionary:
    ...
    var total_duration: float = float(end_time - session_start_time_unix)
    var snapshot: Dictionary = get_integrated_snapshot()

    var average_hrv: float = 0.0
    var average_respiration: float = 0.0

    if total_samples_written > 0:
        average_hrv = hrv_accumulator / float(total_samples_written)
        average_respiration = respiration_accumulator / float(total_samples_written)

    return {
        "session_metadata": {
            "session_id": current_session_id,
            "participant_id": participant_id,
            "protocol_name": snapshot.get("protocol_name", "UNKNOWN_PROTOCOL"),
            "log_directory": csv_file_path.get_base_dir(),
            "csv_file_path": csv_file_path,
            "timestamp_start_unix": session_start_time_unix,
        ...
        "baseline_statistics": {
            "baseline_hrv": baseline_hrv,
            "baseline_respiration_rate": baseline_respiration_rate,
            "session_average_hrv": average_hrv,
            "session_average_respiration": average_respiration
        },
        "validation": {
            "validation_status": "OK" if total_samples_written > 0 else "EMPTY_SESSION",
            "validation_notes": "Contract verified via math engine."
        }
    }


func ingest_replay_snapshot(snapshot: Dictionary) -> void:
    current_session_id = str(snapshot.get("session_id", current_session_id))
    participant_id = str(snapshot.get("participant_id", participant_id))
    sample_index = int(snapshot.get("sample_index", sample_index))

    current_state = str(snapshot.get("guardian_state", current_state))
    confidence = float(snapshot.get("confidence", confidence))
    hrv = float(snapshot.get("hrv", hrv))
    respiration_rate = float(snapshot.get("respiration_rate", respiration_rate))
    reaction_time = float(snapshot.get("reaction_time", reaction_time))
    sleep_quality = float(snapshot.get("sleep_quality", sleep_quality))

    total_samples_written += 1
    hrv_accumulator += hrv
    respiration_accumulator += respiration_rate

    var avg_hrv := hrv_accumulator / float(total_samples_written)
    var avg_resp := respiration_accumulator / float(total_samples_written)

    print("[TELEMETRY MODEL] Replay snapshot processed. Avg HRV: ", avg_hrv, " Avg Resp: ", avg_resp)
    dashboard_data_updated.emit(get_dashboard_summary())
...
func get_dashboard_summary() -> Dictionary:
    ...
    var avg_resp: float = 0.0

    if total_samples_written > 0:
        avg_hrv = hrv_accumulator / float(total_samples_written)
        avg_resp = respiration_accumulator / float(total_samples_written)
