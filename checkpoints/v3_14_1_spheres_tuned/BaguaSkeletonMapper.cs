...
public partial class BaguaSkeletonMapper : Node
{
    ...
    public override void _Ready()
    {
        ...
        GD.Print("[BaguaSkeletonMapper] Skeleton3D found: " + _skeleton.Name + " -- bone mapping starting");
        ...
        // Set up shader material on avatar mesh
        _mesh = FindMesh(GetNode<Node3D>(AvatarPath));
        if (_mesh != null)
        {
            var shader = GD.Load<Shader>("res://assets/avatars/bagua_kappa.gdshader");
            _mat = new ShaderMaterial();
            _mat.Shader = shader;
            _mesh.MaterialOverride = _mat;
            GD.Print("[BaguaSkeletonMapper] Kappa shader applied to mesh");
            // Set default colors so mesh is visible before first frame
            var defaults = new Godot.Collections.Array();
            for (int i = 0; i < 12; i++)
                defaults.Add(new Color(0.196f, 0.506f, 0.51f, 1.0f));
            _mat.SetShaderParameter("bone_colors", defaults);
        }
        else GD.PrintErr("[BaguaSkeletonMapper] MeshInstance3D not found");
    ...
    private MeshInstance3D FindMesh(Node root)
    {
        if (root is MeshInstance3D m) return m;
        foreach (Node child in root.GetChildren())
        {
    ...
    private void OnFrame(BaguaFrameGodot wrapper)
    {
        ...
        for (int i = 0; i < 12; i++)
        {
            int idx = _boneIdx[i];
            if (idx == -1) continue;
            _skeleton.SetBonePosePosition(idx, joints[i].ToVector3());
        }

        // Push kappa heatmap colors to shader
        if (_mat != null)
        {
