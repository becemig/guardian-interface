...
func verify_summary_round_trip(json_summary_path: String, telemetry_model: Node) -> bool:
    print("\n====================================================")
    print("[REPLAY ENGINE] EXECUTING v0.8.4d ROUND-TRIP AUDIT")
    print("====================================================")
    print("[REPLAY ENGINE] Reading historical summary contract: ", json_summary_path)

    if not FileAccess.file_exists(json_summary_path):
        push_error("[REPLAY ENGINE] Target JSON file does not exist.")
        print("====================================================\n")
        return false

    var json_str := FileAccess.get_file_as_string(json_summary_path)
    var json := JSON.new()
    var parse_err := json.parse(json_str)

    if parse_err != OK:
        push_error("[REPLAY ENGINE] Malformed JSON summary.")
        print("====================================================\n")
        return false

    var original_data: Dictionary = json.get_data()
    var original_stats: Dictionary = original_data.get("baseline_statistics", {})

    var target_hrv: float = float(original_stats.get("session_average_hrv", -1.0))
    var target_resp: float = float(original_stats.get("session_average_respiration", -1.0))

    var current_hrv: float = telemetry_model.hrv_accumulator / float(telemetry_model.total_samples_written)
    var current_resp: float = telemetry_model.respiration_accumulator / float(telemetry_model.total_samples_written)

    print("[REPLAY ENGINE] CROSS-COMPARING PIPELINE METRIC REGISTERS:")
    print("HRV Original: ", target_hrv, " | Replayed: ", current_hrv)
    print("Resp Original: ", target_resp, " | Replayed: ", current_resp)

    var hrv_match := is_equal_approx(target_hrv, current_hrv)
    var resp_match := is_equal_approx(target_resp, current_resp)

    if hrv_match and resp_match:
        print("[REPLAY ENGINE] SUCCESS: Round-trip mathematical identity VERIFIED.")
        print("====================================================\n")
        return true

    push_error("[REPLAY ENGINE] Round-trip values diverged.")
    print("====================================================\n")
    return false
